...
import { CheckCircle, AlertCircle, ArrowLeft, FileText, RotateCcw, Trophy, Target, TrendingUp } from "lucide-react"
...
export default function LevelTestPage() {
  ...
  const handleSubmitTest = async () => {
    ...
    try {
      ...
      try {
        const saveResponse = await fetch('/api/level-test-results', {
          method: 'POST',
        ...
        if (!saveResponse.ok) {
          console.error('Failed to save level test results:', await saveResponse.text())
        } else {
          console.log('Level test results saved successfully')
        }
      } catch (saveError) {
        console.error('Error saving level test results:', saveError)
              ...
              <div className={`relative p-8 text-center ${results.passed ? 'bg-gradient-to-r from-green-600 to-emerald-600' : 'bg-gradient-to-r from-red-600 to-rose-600'}`}>
                ...
                <div className="relative z-10">
                  ...
                  <h1 className="text-4xl font-bold text-white mb-2">
                    {results.passed ? '🎉 Level Completed!' : 'Keep Trying!'}
                  </h1>
                  <p className="text-xl text-white/90 mb-4">
                    {results.passed ? 'You Passed the Level Test!' : 'Level Test Not Passed'}
                  </p>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-card rounded-xl p-6 text-center border shadow-sm">
                      ...
                      <div className="text-2xl font-bold text-blue-600 mb-1">{levelTest.passingScore}%</div>
                  ...
                  <Alert className={`border-0 ${results.passed ? 'bg-green-50 text-green-800 dark:bg-green-950 dark:text-green-200' : 'bg-red-50 text-red-800 dark:bg-red-950 dark:text-red-200'}`}>
                    ...
                    <AlertDescription className="text-base">
                      {results.passed
                        ? "Excellent work! You've successfully completed this level. Continue to the next level to progress further in your learning journey!"
                        : `You need ${levelTest.passingScore}% to pass. Review the level content and try again.`
                      }
                    </AlertDescription>
                  </Alert>
