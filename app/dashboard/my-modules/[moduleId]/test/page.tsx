...
import { CheckCircle, AlertCircle, ArrowLeft, FileText, RotateCcw, Trophy, Target, TrendingUp, Award } from "lucide-react"
...
export default function ModuleTestPage() {
  ...
  const [moduleTest, setModuleTest] = useState<ModuleTest | null>(null)
  ...
  const fetchModuleTest = async () => {
    try {
      // Get module test using our Prisma API
      const testResponse = await fetch(`/api/module-tests?moduleId=${moduleId}`)
      if (!testResponse.ok) {
        console.error('Module test not found:', testResponse.status)
        router.push(`/dashboard/my-modules/${moduleId}`)
        return
      }

      const testData = await testResponse.json()
      setModuleTest(testData)
  ...
  const handleSubmitTest = async () => {
    ...
    try {
      ...
      const passed = score >= moduleTest.passingScore
      ...
      try {
        const saveResponse = await fetch('/api/module-test-results', {
          method: 'POST',
        ...
        if (!saveResponse.ok) {
          console.error('Failed to save module test results:', await saveResponse.text())
        } else {
          console.log('Module test results saved successfully')
        }
      } catch (saveError) {
        console.error('Error saving module test results:', saveError)
              ...
              <div className={`relative p-8 text-center ${results.passed ? 'bg-gradient-to-r from-green-600 to-emerald-600' : 'bg-gradient-to-r from-red-600 to-rose-600'}`}>
                ...
                <div className="relative z-10">
                  ...
                  <h1 className="text-4xl font-bold text-white mb-2">
                    {results.passed ? '🎉 Module Completed!' : 'Keep Trying!'}
                  </h1>
                  <p className="text-xl text-white/90 mb-4">
                    {results.passed ? 'You Passed the Final Test!' : 'Module Test Not Passed'}
                  </p>
                ...
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-card rounded-xl p-6 text-center border shadow-sm">
                      ...
                      <div className="text-2xl font-bold text-blue-600 mb-1">{moduleTest.passingScore}%</div>
                  ...
                  <Alert className={`border-0 ${results.passed ? 'bg-green-50 text-green-800 dark:bg-green-950 dark:text-green-200' : 'bg-red-50 text-red-800 dark:bg-red-950 dark:text-red-200'}`}>
                    ...
                    <AlertDescription className="text-base">
                      {results.passed
                        ? "Congratulations! You've successfully completed this entire module. Your certificate will be available within 48 hours. Check your profile for updates!"
                        : `You need ${moduleTest.passingScore}% to pass. Review all module content and try again.`
                      }
                    </AlertDescription>
                  </Alert>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 pt-4">
                    ...
                    <Button
                      onClick={() => router.push('/dashboard/my-modules')}
        ...
        <div className="max-w-2xl w-full">
          <Card className="shadow-lg">
            <CardHeader className="text-center bg-muted/50">
              ...
              <CardDescription>{moduleTest.description}</CardDescription>
            </CardHeader>

            <CardContent className="p-8 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-muted/50 rounded-lg p-4 text-center border">
                  <div className="text-2xl font-bold text-primary mb-1">{moduleTest.totalQuestions}</div>
                  <div className="text-sm text-muted-foreground">Questions</div>
                </div>
                <div className="bg-muted/50 rounded-lg p-4 text-center border">
                  <div className="text-2xl font-bold text-primary mb-1">{moduleTest.passingScore}%</div>
                  <div className="text-sm text-muted-foreground">Passing Score</div>
                </div>
              </div>
